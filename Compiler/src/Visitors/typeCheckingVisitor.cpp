...
void typeCheckingVisitor::visitTerminalExpression(TerminalExpression *terminal) {
...
void typeCheckingVisitor::visitBinaryExpression(BinaryExpression *expression) {
    ...
    if(lhsType == rhsType){
        ...
        throw std::invalid_argument("Binary operation with mismatching types: " + lhsType + " and " + rhsType + "\n");
    }
    
}

void typeCheckingVisitor::visitVariableAssignmentExpression(VariableAssignmentExpression *variable) {
    variable->getVariable()->accept(this);
    if(variable->getType() != type){
        throw std::invalid_argument("variable assignment operation with mismatching types: " + variable->getType() + " and " + type + "\n");
    }else{
        typeTable.insert({variable->getVariable()->getName(), variable->getType()});
    }
}

void typeCheckingVisitor::visitVariableExpression(VariableExpression *variable) {
...
void typeCheckingVisitor::visitFunctionDeclaration(FunctionDeclaration *FuncDeclExpr) {
