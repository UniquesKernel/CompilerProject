...
public:
  BinaryExpression(std::unique_ptr<BaseExpression> lhs, char op,  std::unique_ptr<BaseExpression> rhs)
      : lhs(std::move(lhs)), op(op), rhs(std::move(rhs)) {  
        // if(lhs->getType() == rhs->getType()){
        //   type=lhs->getType();
        // }else{
        //   throw std::invalid_argument("Binary opperator must have matching types: " + TerminalTypeStrings[lhs->getType()] + " " + op + " " + TerminalTypeStrings[rhs->getType()]);
        // }
      }
