  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \begin{tikzpicture}
        ...
        \node[draw, rectangle, minimum width=3.5cm, below=0.5cm of
          typeChecker, align=center] (borrowChecker) {\borrowChecker};
        \draw[->] (typeChecker) -- (borrowChecker);
      ...
      \draw[->] (parser) -- (static) node[midway, right] {AST};
      \draw[->] (static) -- (codeGen) node[midway, right] {AST};
      \draw[->] (codeGen) -- (object) node[midway, right] {IR Code};
      \draw[->] (object) -- (exec) node[midway, right] {Binary File}; 
    \end{tikzpicture}
  \end{center}
